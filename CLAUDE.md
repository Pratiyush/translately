...
12. **Licensing: MIT outbound, CLA inbound.** End users consume Translately under the [MIT License](LICENSE). Contributors accept the [Contributor License Agreement](CLA.md) when they tick the CLA checkbox in the PR template — a copyright + patent *license* (contributor retains ownership) that lets the project maintainer sublicense or relicense the combined Work. No PR merges without the ticked box; review-reject any PR that omits or strikes the line. Do not introduce third-party code under AGPL/GPL or any licence incompatible with MIT redistribution.
